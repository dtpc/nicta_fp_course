...
hex ::
  Parser Char
hex = 
  readHex <$> replicateA 4 (satisfy isHexDigit) >>= (\(Full a) -> valueParser (chr a))
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma a b c =
  betweenCharTok a b (sepby c (charTok ','))
