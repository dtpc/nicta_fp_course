...
findRight p (ListZipper l m r) =
  ...
  in case b of
    Nil     -> IsNotZ
    (x:.xs) -> IsZ $ ListZipper (reverse a ++ (m:.l)) x xs
...
moveLeftLoop (ListZipper Nil m r) = 
  let (x :. xs) = reverse (m :. r)
  in ListZipper xs x Nil
moveLeftLoop z = 
  let IsZ lz = findLeft (const True) z
  in lz
...
moveRightLoop (ListZipper l m Nil) = 
  let (x :. xs) = reverse (m :. l)
  in ListZipper Nil x xs
moveRightLoop z = 
  let IsZ lz = findRight (const True) z
  in lz
...
swapLeft (ListZipper Nil _ _) =
  IsNotZ
swapLeft (ListZipper (x:.xs) m r) =
  IsZ $ ListZipper (m:.xs) x r
...
swapRight (ListZipper _ _ Nil) =
  IsNotZ
swapRight (ListZipper l m (x:.xs)) =
  IsZ $ ListZipper l x (m:.xs)
...
moveLeftN' n z@(ListZipper l _ _) 
  ...
  | otherwise    = let IsZ x = moveLeftN n z
                   in Right x
...
moveRightN' n z@(ListZipper _ _ r) 
  ...
  | otherwise    = let IsZ x = moveRightN n z
                   in Right x
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth i z@(ListZipper l _ r)
    | i < 0 || i > ei = IsNotZ
    | otherwise       = moveLeftN (mi - i) z
  where mi = length l 
        ei = mi + length r
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end z@(ListZipper l _ r) = 
  let n = length l + length r
      IsZ x = nth n z
  in x
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft a (ListZipper l m r) =
  ListZipper (m :. l) a r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight a (ListZipper l m r) =
  ListZipper l a (m :. r)
...
-- /Tip:/ Use @List#repeat@.
  pure a =
    let as = repeat a
    in ListZipper as a as
  
-- /Tip:/ Use `zipWith`
  ListZipper fl fm fr <*> ListZipper l m r =
    ListZipper (zipWith ($) fl l) (fm m) (zipWith ($) fr r) 
...
instance Extend ListZipper where
  f <<= z = 
    z' where
      next a = case nth a z of
                IsZ x  -> Full (f x, a+1)
                IsNotZ -> Empty
      IsZ z' = nth (index z) -<< (fromList $ unfoldr next 0)
